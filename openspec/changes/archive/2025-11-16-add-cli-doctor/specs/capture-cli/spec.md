...
The capture CLI MUST provide a `doctor` subcommand that validates runtime prerequisites for `chissu-cli enroll` and the PAM module without mutating state, emitting both human-readable and `--json` structured reports plus a non-zero exit code on any failed check.
...
- **WHEN** `doctor` resolves `video_device` via the shared loader defaults/overrides
