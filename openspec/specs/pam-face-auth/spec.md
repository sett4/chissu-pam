...
- **WHEN** `/etc/chissu-pam/config.toml` defines `video_device = "/dev/video2"`, `warmup_frames = 6`, and `embedding_store_dir = "/srv/chissu/embeddings"` (or the legacy `descriptor_store_dir`)
