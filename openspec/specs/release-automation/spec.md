...
- **THEN** a workflow runs on `ubuntu-latest`, builds Debian-compatible packages inside release-specific Debian/Ubuntu container images, and invokes `build/package-deb.sh` with explicit suite and artifact-label values for each target release
...
- **AND** uploads each `.deb` file as a release asset with clear release-specific filenames (e.g., `chissu-pam_1.2.3_ubuntu-24.04_amd64.deb`)
...
- **THEN** they learn to push a `v<MAJOR>.<MINOR>.<PATCH>` or `v<MAJOR>.<MINOR>.<PATCH>-<prerelease>` tag, wait for the workflow, and confirm release-specific `.deb`/`.rpm` assets on GitHub Releases, including notes about required permissions and how to re-run a failed job.
