...
- **AND** it invokes `dpkg-buildpackage -us -uc` (or equivalent debhelper tooling) to emit a `.deb` file into `dist/` whose filename embeds the target release label and version (e.g., `chissu-pam_<version>_debian-12_amd64.deb`)
...
- **THEN** the script reuses the same staging workflow but writes Ubuntu-specific control metadata for the selected suite/codename before calling `dpkg-buildpackage`
- **AND** the resulting artifact lands in `dist/` as `chissu-pam_<version>_ubuntu-<release>_amd64.deb`
...
#### Scenario: Maintainer overrides target suite and artifact label
- **WHEN** the maintainer passes `--suite noble --artifact-label ubuntu-24.04`
- **THEN** the generated changelog metadata targets the `noble` suite
- **AND** the resulting artifact filename uses `ubuntu-24.04` so release assets identify the exact binary-compatible distro release
